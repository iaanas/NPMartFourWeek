...
class OrdersDataAdapterTwo extends ArrayAdapter<Order> {
    ...
    public View getView(int position, View convertView, @NonNull ViewGroup parent) {
        ...
        assert item != null;
        vh.pos_name.setText(item.getPositionOrders().get(0).getPosition().getName());
        vh.pos_quantity.setText(item.getPositionOrders().get(0).getQuantity().toString());
        vh.pos_price.setText(item.getPositionOrders().get(0).getPosition().getPrice().toString());
        vh.pos_total.setText(item.getPositionOrders().get(0).getPriceTotal().toString());
